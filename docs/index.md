...
* SRS 03: UART Communication between STM32s - [UART Demo](https://youtu.be/eALjC2jrxNI) - The two MCUs communicate with each other via UART. One MCU processes and tracks the user input and modes of operation and sends them to the second MCU via UART
...
During this project, the most new thing I challenged myself to learn was how to design, CAD, and laser cut our box. Bhavya and two of my MEAM friends were very helpful throughout the process and I couldn’t have done it without them!  I feel that learning this skill will open a lot of new opportunities for me to prototype or explore more versatile paths. I also worked on designing two analog modules that were not implemented due to them not working reliably by the time necessary. The first was a Voltage Controlled Amplifier. At the time, we were having issues with our TPA2012 providing too much gain and our output being very loud. As a result, it led us to consider building a VCA that could reduce the gain on our signal. Turns out not many people want to reduce sound signals before output, so there was little documentation for such a circuit. I decided to design one myself by making a CD amplifier with a transistor in the deep triode region as a load. I got to design a boost converter to operate the triode-transistor which I really liked. Overall, the circuit worked exactly as expected when we used the frequency generator as input: I was able to vary the gain by varying the input voltage to the boost converter. Unfortunately, it failed when we routed the actual signal as input from the VCO. Also, because human hearing is on the decibel scale, it was difficult to discern the changes in sound amplitude that we could see on the oscilloscope, so the amplifier was pretty useless from a user perspective. Still, I was super proud of myself that I managed to theorize and design something successfully based on previous coursework. I wish I was more comfortable designing with Op-Amps, because I think that would have been the more reliable way to do it. I also tried to repurpose a 3190 lab (the summing amplifier) but ran into issues with oscillator / input design. Also, our code was nearing completion at this point so we wanted to prioritize debugging code over adding new features that could potentially break everything. Lastly, I worked on selecting the parts for the output & microphone stage. While looking at different output and input amps, I did some research into I2S as STM was heavily recommended to us because of it. I also got far more comfortable reading and comparing datasheets of different parts to decide what was best for us. Lastly, while I was not the main person working on code, I still got to strengthen a lot of the core concepts we learned in class-  timers, interrupts, serial communication, and ADC- by seeing them applied to a new controller.
